...
// 1. Buscamos el borrador por el token
$url = "http://{$serverIp}:{$dbPort}/rest/v1/registros_abandonados?token=eq." . urlencode($token);
$ch = curl_init($url);
...
if (is_array($datos) && count($datos) > 0) {
    $borrador = $datos[0];
    $emailRecuperado = $borrador['email'];

    // 2. RESTAURAMOS LA SESIÓN COMPLETA
    ...
    // 3. RESET DE "CORREO_ENVIADO" (Para poder rescatarlo otra vez si vuelve a abandonar)
    $urlUpdate = "http://{$serverIp}:{$dbPort}/rest/v1/registros_abandonados?email=eq." . urlencode($emailRecuperado);
    $chUpd = curl_init($urlUpdate);
    curl_setopt_array($chUpd, [
        CURLOPT_CUSTOMREQUEST => 'PATCH',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_TIMEOUT => 2,
        CURLOPT_POSTFIELDS => json_encode(['correo_enviado' => false]),
        CURLOPT_HTTPHEADER => [
    ...
    // 4. Redirigimos al paso donde se quedó
