...
// 2. PETICIONES AJAX PARA AÑADIR/EDITAR/ELIMINAR SERVICIOS
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] !== 'update_space') {
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Content-Type: application/json',
        'apikey: ' . $_ENV['SERVICE_APIKEY'], // PERMISOS ADMIN
        'Authorization: Bearer ' . $_ENV['SERVICE_APIKEY']
    ));
...
if (isset($_GET['id']) || isset($_POST['id'])) {
    ...
    curl_setopt_array($chGet, array(
        ...
        CURLOPT_HTTPHEADER => array(
            'Content-Type: application/json',
            'apikey: ' . $_ENV['SERVICE_APIKEY'], // Usamos Service Key para garantizar lectura como admin
            'Authorization: Bearer ' . $_ENV['SERVICE_APIKEY'],
...
<head>
    ...
    <title>Editar Espacio - Admin</title>
    ...
    <style>
        :root {
            --primary-admin: #dc3545;
            --primary-admin-hover: #b02a37;
        }
        ...
        .btn-guardar:hover {
            background-color: var(--primary-admin-hover);
            color: white;
        }
    ...
    <div class="container">
        <div class="form-container">
            <div class="header-custom">
                <h3 class="fw-bold m-0"><i class="fas fa-edit me-2"></i>Edición de Espacio (Administrador)</h3>
                <div class="badge-est">
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            ...
                            setTimeout(() => {
                                window.location.href = window.location.href.split('?')[0] + '?id=<?php echo htmlspecialchars($espacio['id']); ?>';
                            }, 1000);
                        } else {
