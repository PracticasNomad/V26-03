...
if (isset($destiny) && isset($_SESSION['verification_code_guest'])) {
    $mail = new PHPMailer(true);
    $emailBody = "
    Hola $nombre,<br><br>
    Gracias por registrarte.<br>
    Para completar tu registro, por favor utiliza el siguiente código de verificación:<br><br>
    🔐 Código de verificación: " . $_SESSION['verification_code_guest'] . "<br><br>
    ...
    try {
        ...
        $mail->send();
        header('Location: ../verificarRegister.php?status=ok');
    } catch (Exception $e) {
        header('Location: ../verificarRegister.php?status=error:' . $mail->ErrorInfo);
    }
} else {
    header('Location: ../verificarRegister.php?status=error:missing_parameters');
}
